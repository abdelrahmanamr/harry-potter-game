...
public class FirstTask extends Task {
	...
	private ArrayList<Point> OldmarkedCells;
	public ArrayList<Point> getOldmarkedCells() {
		return OldmarkedCells;
	}

	public void setOldmarkedCells(ArrayList<Point> oldmarkedCells) {
		OldmarkedCells = oldmarkedCells;
	}
	...
	public void fire() {
		...
		Cell[][] map = getMap();
		Attacked = true;
		setOldmarkedCells(getMarkedCells());
	...
	public void finalizeAction() throws IOException {
		...
		if (current.getLocation().x == 4 && current.getLocation().y == 4) {
			...
			JOptionPane.showMessageDialog(null, ((Wizard)getCurrentChamp()).getName() +" has qualified to the second task.");
	...
	}public static void DragonEffect(){
		try {
		    File yourFile;
		    ...
		    DataLine.Info info;
		    Clip clip;
	        yourFile = new File("src/Dragon.wav");
